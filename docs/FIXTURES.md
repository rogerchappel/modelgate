...
The object shapes below are closed. Unknown provider, model, model `cost`, or
route keys are rejected with an indexed configuration path. This prevents
misspellings such as `enabledd` or `monthlyBudgteUsd` from silently disabling a
safety or budget setting.
